...
def create_google_sheet_copy(recipient, user_id):
    """Create a unique Google Sheet for each recipient."""
    try:
        creds = authenticate()
        ...
        spreadsheet = {
            'properties': {
                'title': f"Supplier Form for {recipient}"
            }
        }
        sheet = service.spreadsheets().create(body=spreadsheet).execute()
        sheet_link = f"https://docs.google.com/spreadsheets/d/{sheet['spreadsheetId']}/edit"
        ...
        logger.error(f"Error creating Google Sheet: {err}")
        return None
...
def create_google_sheet_copy(recipient_email: str, user_id: str, expiration_days: int = 2):
    """Creates a copy of the Google Sheet, shares it, and sets a countdown for access expiration."""
    try:
        ...
        # 🔹 Calculate expiration time (e.g., 2 days from now)
        creation_time = datetime.datetime.now()
        expiration_time = creation_time + datetime.timedelta(days=expiration_days)

        # ✅ Log the details (you could store this in MySQL instead)
        ...
        def revoke_access():
            ...
            try:
                # Revoke "anyone" permission by deleting it
                drive_service.permissions().delete(
                    fileId=new_sheet_id,
                    permissionId="anyoneWithLink"
                ).execute()
                logger.info(f"Access revoked for Sheet: {new_sheet_id}")
...
def create_google_form_copy(recipient_email, user_id):
    """Creates a copy of the template Google Form and shares it with the recipient."""
    try:
        drive_service = get_drive_service()  # ✅ Initialize Google Drive service

        # Google Form template ID
        template_form_id = "1LHlFrm5IKDMXZoGCrBQB4R35ZYepuLsw5NW3VVsxzDI"

        print(f"🔄 Creating form copy for {recipient_email}...")  # Debugging output

        # Copy the Google Form in Drive
        copied_form = drive_service.files().copy(
            fileId=template_form_id,
            body={
                "name": f"Supplier Declaration Form - {recipient_email}",
                "parents": ["14BLdtpwodH-p75biqvDNXYRVvm-TIUojyoi0V1r-LOXeI74boE-Tta1ZhklbG0WhSckAMnIa"]  # Folder ID
            },
        ).execute()

        if not copied_form or "id" not in copied_form:
            print(f"❌ Error: Form copy failed for {recipient_email}")
            return None

        form_id = copied_form["id"]
        form_link = f"https://docs.google.com/forms/d/{form_id}/edit"
        print(f"✅ Form Created: {form_link}")

        # Share the form with the supplier (Give Editor access)
        drive_service.permissions().create(
            fileId=form_id,
            body={"type": "user", "role": "writer", "emailAddress": recipient_email},
            fields="id",
        ).execute()

        print(f"✅ Form shared with {recipient_email}")

        return form_link

    except Exception as e:
        print(f"❌ Error creating Google Form for {recipient_email}: {e}")
        return None
...
def send_email():
    ...
    if request.method == 'POST':
        ...
        for recipient in recipients:
            ...
            # Use the create_google_sheet_copy function to get the unique link for each recipient
            sheet_links[recipient] = create_google_sheet_copy(recipient, user_id)
            form_links[recipient] = create_google_form_copy(recipient, user_id)
...
def oauth_callback():
    ...
    try:
        ...
        for recipient in recipients:
            ...
            if sent_status:
                cursor.execute("""
                    INSERT INTO email_logs (user_id, recipient, subject, sheet_link, form_link, sent_time)
                    VALUES (%s, %s, %s, %s, %s, NOW())
                """, (user_id, recipient, subject, sheet_link, form_link))
                conn.commit()
