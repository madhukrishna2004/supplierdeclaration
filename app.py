...
def register():
    """Register user only if exists in users_main table and not already registered in users table."""
    if request.method == 'POST':
        ...
        try:
            ...
            # 1. Check if email exists in users_main
            cursor.execute("""
                SELECT * FROM users_main 
                WHERE email = %s
            """, (email,))  # <-- Corrected single-value tuple with trailing comma
            main_user = cursor.fetchone()

            if not main_user:
                flash("Registration denied. You are not authorized or not listed in our system.", "danger")
